...
public class ReportUnbanCmd implements CommandExecutor {
            ...
            if(target.isModMode()){
                if(args.length == 1){
                    if(Bukkit.getPlayer(args[0]) != null){
                        ...
                        MonitorPlayer unbannedPlayer = MonitorPlayer.getMonitorProfile(unbanned);
                        unbannedPlayer.unbanReport();
                        player.sendMessage("§c[Monitor] §aYou unbanned §e" + unbanned.getName() + "§a from using reports");
                        unbanned.sendMessage("§aYou are now unbanned from the report system.");
                    }else {
